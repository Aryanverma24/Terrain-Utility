...
const getLawyers = async (req, res) => {
  try {
    const lawyers = await User.find({ role: 'lawyer' }).select('_id username email');
...
const reuploadUserDocumentHandler = asyncHandler(async (req, res) => {
  ...
  if (subDoc.status !== 'rejected')
    return res
      .status(400)
      .json({ message: `Current status is '${subDoc.status}', not 'rejected'` });

  // Upload new file to Cloudinary
  const oldPublicId = subDoc.file?.cloudinary
    ? extractPublicId(subDoc.file.cloudinary)
    : null;
