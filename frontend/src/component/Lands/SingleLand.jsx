        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-bold text-emerald-600 mt-2">
              {Number(conversions[selectedUnit] || 0).toLocaleString(undefined, {
...
const SingleLand = ({ onOpenChat }) => {
  ...
  const isOwner =
    user?._id?.toString() === (land?.owner?._id?.toString() || land?.owner?.toString());
  ...
  const handleExploreMore = () => {
    ...
    if (!actuallyInterested) {
      toast.warning('You have to show interest first to explore more about this land');
  ...
  const handleOwnershipHistory = () => {
    ...
    toast.info(
      'Ownership history feature coming soon! This will show the complete transfer history of this property.',
    );
  ...
  const handleApprove = async () => {
    ...
    const isGeoDone = land.geoVerification?.status !== 'pending';

    const isLawyerDeclared = land.geoVerification?.lawyerDeclaration?.accepted === true;
  ...
  const handleResubmit = async () => {
    if (!window.confirm('Are you sure you want to resubmit this land for re-evaluation?'))
      return;
  ...
  const handleReuploadDocument = async (parentDocumentId, docId, file) => {
    ...
    try {
      ...
      const res = await API.put(`/api/documents/${docId}/reupload`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteReview = async (reviewUserId) => {
    ...
    if (!currentUserId || currentUserId !== reviewUserId)
      return toast.error('You can delete only your review.');
    try {
  ...
  if (error)
    return <div className="flex justify-center pt-[7rem] text-red-400">{error}</div>;
  ...
  const hasRejectedDocs =
    Array.isArray(fullDocs) && fullDocs.some((doc) => doc.status === 'rejected');
          ...
          <div className="mb-8">
            <div className="bg-white/60 backdrop-blur-sm rounded-xl px-6 py-3 shadow-sm border border-emerald-100">
              <nav className="flex items-center space-x-3 text-sm">
                <Link
                  to="/"
                  className="flex items-center text-emerald-600 hover:text-emerald-700 transition-all duration-200 font-medium"
                >
                    ...
                    <div className="bg-white rounded-2xl p-8 w-full max-w-4xl max-h-[80vh] overflow-hidden shadow-2xl">
                        ...
                        {interestedUsersData.length > 0 ? (
                                      ...
                                      <td className="py-4 px-4">
                                        <div className="flex justify-center gap-2">
                                          <button
                                            onClick={() =>
                                              toast.info(
                                                `Chat with ${u.username} feature coming soon!`,
                                              )
                                            }
                                          ...
                                          <button
                                            onClick={() =>
                                              toast.info(
                                                `View ${u.username} profile feature coming soon!`,
                                              )
                                            }
                          ...
                          <div className="text-center py-12">
                            ...
                            <p className="text-gray-400 text-sm mt-2">
                              Interested users will appear here once they mark this
                              property as interested.
                            </p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="mt-6 pt-6 border-t border-gray-200">
                        <div className="flex justify-between items-center">
                          <p className="text-sm text-gray-500">
                            Total {interestedUsersData.length}{' '}
                            {interestedUsersData.length === 1 ? 'person' : 'people'}{' '}
                            interested
                          </p>
                          <button
                            onClick={() => setShowInterestedUsersModal(false)}
                        ...
                        {landCases.length === 0 ? (
                          ...
                          landCases.map((c) => (
                            <div key={c._id} className="border rounded-lg p-3 mb-3">
                              <p>
                                <strong>Buyer:</strong> {c.buyerId?.username}
                              </p>
                              <p>
                                <strong>Lawyer:</strong> {c.lawyerId?.username}
                              </p>
                ...
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <div className="pr-12">
                      ...
                      <p className="text-lg font-bold text-gray-900">
                        {land.city && land.state
                          ? `${land.city}, ${land.state}`
                          : land.city || land.state || 'Location Not Available'}
                      </p>
                      <p className="text-sm text-gray-600 mt-1">
                        Pincode: {land.pincode || 'N/A'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="group relative p-5 bg-gradient-to-br from-teal-50 via-cyan-50 to-sky-50 rounded-2xl border border-teal-200 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    ...
                    <div className="pr-12">
                      ...
                      <p className="text-lg font-bold text-gray-900">
                        {land.ownerName || 'Owner Information Not Available'}
                      </p>
                    </div>
                  </div>
