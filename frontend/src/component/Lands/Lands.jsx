  ...
  const applyFiltersAndPagination = (landsToFilter = allLandsData) => {
    ...
    if (filters.landType) {
      filtered = filtered.filter(land => 
        land.landtype?.toLowerCase() === filters.landType.toLowerCase()
      );
    }
    ...
    if (filters.minPrice) {
      filtered = filtered.filter(land => 
        Number(land.price) >= Number(filters.minPrice)
      );
    }
    
    if (filters.maxPrice) {
      filtered = filtered.filter(land => 
        Number(land.price) <= Number(filters.maxPrice)
      );
    }
    
    if (filters.minArea) {
      filtered = filtered.filter(land => 
        Number(land.area) >= Number(filters.minArea)
      );
    }
    
    if (filters.maxArea) {
      filtered = filtered.filter(land => 
        Number(land.area) <= Number(filters.maxArea)
      );
    }
    ...
    const uniqueTypes = [...new Set(filtered.map(land => land.landtype).filter(Boolean))];
    const avgPrice = filtered.length > 0 
      ? filtered.reduce((sum, land) => sum + (Number(land.price) || 0), 0) / filtered.length 
      : 0;
